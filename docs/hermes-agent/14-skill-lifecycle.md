...
이 글에서 다루는 내용: [#4](./04-tools-system)에서 스킬이 "재사용 가능한 절차"이고 에이전트가 스스로 저장한다고 봤다. 그런데 스킬을 계속 만들기만 하면 쌓이기만 한다. 안 쓰는 스킬, 낡은 스킬, 겹치는 스킬이 늘어난다. Hermes에는 이걸 자동으로 정리하는 curator라는 시스템이 있다. [#5](./05-memory-and-sessions)에서 본 "삭제 대신 비활성화" 철학이 스킬에도 적용된다.
