...
이 글에서 다루는 내용: 에이전트가 다른 에이전트에게 일을 시키는 방법. 왜 위임이 컨텍스트를 지키는 전략인지, 그리고 이걸로 "구현은 A에게, 리뷰는 B에게" 같은 dispatcher 패턴을 어떻게 만드는지 살펴본다.
...
#3에서 `KANBAN_GUIDANCE` 프롬프트를 봤다. 그것이 바로 Kanban worker로 스폰된 에이전트가 받는 작업 규약이다. (Kanban 심화는 별도 편에서 다룬다.)
