...
> 이번 편에서는 Hermes가 "기억"을 어떻게 다루는지 본다. 대화가 저장되는 위치, 과거 대화를 다시 찾는 방식, 영구 메모리에 글자수 제한을 두는 이유까지 따라간다. 마지막에는 메모리 챗봇을 만들 때 챙길 점도 짚는다.
...
위 4개는 큰 그림이고, `hermes_state.py`의 `SCHEMA_SQL`을 열면 테이블이 더 있다. 에이전트용 DB를 직접 설계할 때 참고가 되도록 실제 스키마를 본다.
...
#1에서도 짚었지만 다시 정리해 둔다. 기록은 로컬, 추론은 클라우드다.
...
세션이 로컬 SQLite에 산다는 사실에는 한 가지 따라오는 성질이 있다. 세션은 특정 플랫폼에 묶이지 않는다. CLI에서 시작한 대화든, 텔레그램에서 시작한 대화든, 같은 `state.db`의 세션 레코드일 뿐이다. 그래서 README가 말하는 "텔레그램에서 클라우드 VM의 작업을 이어서 본다"가 가능하다. 게이트웨이 프로세스 하나가 여러 플랫폼 어댑터(Telegram, Discord, Slack...)를 동시에 물고, 모두 같은 세션 저장소를 공유한다.

이게 작동하는 이유는 [#8 게이트웨이](./08-gateway)에서 본 라우팅 구조 덕분이다. 각 플랫폼 메시지는 어댑터를 거쳐 들어오지만, 도착 지점은 공통의 에이전트 루프와 공통의 세션 저장소다. 플랫폼은 입출력 채널일 뿐, 대화의 정체성(세션)은 그 아래 레이어에 있다. 그래서 한 채널에서 진행한 작업의 컨텍스트가 다른 채널에서도 같은 세션으로 이어진다.
...
마지막으로 정리해 둔다. Hermes에는 "압축"이 두 종류 있고 대상이 다르다.
...
다음 편(#6): 지금까지 살펴본 구조를 뒤집어서, "내가 에이전트를 직접 만든다면 Hermes의 어떤 결정을 참고할 것인가"를 7가지 설계 교훈으로 정리해 둔다.
